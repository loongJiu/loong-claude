...
**固定话术（全量流程）**：
> 「技术规范已写入 `<路径>`。请审阅——如需修改请直接说明；如已确认，回复 **✓** 即可开始实施。」

**固定话术（快速通道）**：
> 「方案如上。如需调整请直接说明；确认后回复 **✓** 开始实施。」

**阻断点**：只有收到明确批准（包括 ✓ 或等效的确认语），方可进入 Step 9。若需修改则返回更新并重新执行 Step 7。
...
- **全量流程**：生成分步实施计划（列出每一步的目标文件、改动类型、验证方式），获确认后开始编码。

实施计划的粒度以"每步可独立验证"为标准，不强制使用特定技能或工具。
